...
//   3. Detects when Khalti redirects back to parkingapp://payment/callback
...
export default function KhaltiWebView() {
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // Core callback handler. Called from BOTH onShouldStartLoadWithRequest (fires
  // first, reliable on Android for custom schemes) and onNavigationStateChange
  // (fallback). The handledCallback guard prevents it running twice.
  // ────────────────────────────────────────────────────────────────────────────
  const processCallbackUrl = (url: string) => {
    if (!url || handledCallback) return;
    if (!url.startsWith(KHALTI_CONFIG.returnUrl)) return;
    ...
    // Status is Completed (or absent) — verify server-side regardless.
    verifyPayment(callbackPidx);
  };

  // onShouldStartLoadWithRequest: intercept the custom-scheme redirect BEFORE the
  // WebView tries (and fails) to load it. Returning false stops the load.
  const handleShouldStartLoad = (request: { url: string }): boolean => {
    if (request.url.startsWith(KHALTI_CONFIG.returnUrl)) {
  ...
  // Fallback for platforms/cases where onShouldStartLoadWithRequest doesn't fire.
  ...
  return (
    <View style={styles.container}>
      ...
      <WebView
        ...
        originWhitelist={['https://*', 'parkingapp://*']}
